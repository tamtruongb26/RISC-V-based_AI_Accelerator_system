...
module accelerator_master_stream_v2_0_M00_AXIS #(
    ...
    input  wire [9:0]                        pi_num_transfers,
    ...
    wire      is_last   = (tx_count == pi_num_transfers - 10'd1) & handshake;

    always @(posedge M_AXIS_ACLK) begin
        if (!M_AXIS_ARESETN) begin
            tx_count <= 10'd0;
        end else if (handshake) begin
            if (tx_count == pi_num_transfers - 10'd1)
                tx_count <= 10'd0;            // wrap về 0 cho tile kế
            else
    ...
    assign M_AXIS_TLAST  = (tx_count == pi_num_transfers - 10'd1) & M_AXIS_TVALID;

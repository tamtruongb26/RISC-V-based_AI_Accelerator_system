...
int main()
{
	...
	if ((ret = mnist_load("train-images.idx3-ubyte", "train-labels.idx1-ubyte", &data, &cnt)) != 0)
	...
	int max_weight_width = 24;

	char wtemp[max_weight_width];

	while (fgets(wtemp, max_weight_width, fw))
	{
        w = atof(wtemp);
	...
	for (p = 0; p < itterations; p++)
	{
		...
		for (n = 0; n < f; n++) M_hidden_1[n] = 0.0;
		for (n = 0; n < s; n++) M_hidden_2[n] = 0.0;
		for (n = 0; n < o; n++) M_output[n] = 0.0;
	...
	if (argmax(M_output, 10) == (data+p)->label)
    	count_correct_value++;

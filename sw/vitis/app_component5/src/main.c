...
/* ── No-reboot loop helpers ──────────────────────────────────────────────
 * Reboot mỗi ảnh fragile (image 2+ không boot lại được dù đã rewire reset).
 * Thay bằng: boot PicoRV32 1 lần, FW tự lặp ảnh qua handshake GO/PASS. */
#define BOOT_ATTEMPTS        4
#define BOOT_CONFIRM_POLLS   2000000u

static int boot_pico_once(void)
{
    for (int attempt = 0; attempt < BOOT_ATTEMPTS; attempt++) {
        Xil_Out32(RAAS_PS_SW_RESET_BASE + RAAS_SW_RESET_OFFSET, RAAS_SW_RESET_HOLD);
        (void)Xil_In32(RAAS_PS_SW_RESET_BASE + RAAS_SW_RESET_OFFSET); /* AXI flush */
        usleep(100);
        ...
        for (uint32_t i = 0; i < BOOT_CONFIRM_POLLS; i++) {
            Xil_DCacheInvalidateRange(DDR_PS_BASE + LENET_DDR_MAILBOX_OFF, 4);
            if (Xil_In32(DDR_PS_BASE + LENET_DDR_MAILBOX_OFF) == RAAS_MBX_STARTED)
                return 1;
        }
    }
    return 0;
...
/* Ảnh đã ở DDR → clear predicted → ghi GO → chờ FW trả PASS/error (không reset) */
static uint32_t trigger_and_wait(void)
{
    Xil_Out32(DDR_PS_BASE + LENET_DDR_PREDICTED_OFF, 0xFFFFFFFFu);
    Xil_Out32(DDR_PS_BASE + LENET_DDR_PREDICTED_SW_OFF, 0xFFFFFFFFu);
    Xil_Out32(DDR_PS_BASE + LENET_DDR_LAYER_DBG_OFF, 0x0);
    Xil_Out32(DDR_PS_BASE + LENET_DDR_MAILBOX_OFF, RAAS_MBX_GO);   /* GO cuối cùng */
    Xil_DCacheFlushRange(DDR_PS_BASE + LENET_DDR_MAILBOX_OFF, 64);
    return wait_mailbox_change();
}

/* Stage ảnh mới vào DDR rồi GO + chờ kết quả. */
static uint32_t run_image(const uint32_t *image)
{
    write_words_to_ddr(LENET_DDR_IMAGE_OFF, image, 392);
...
int main(void)
{
    ...
    /* 5. Boot PicoRV32 MỘT lần — no-reboot loop (FW tự lặp ảnh qua GO/PASS).
     * Tránh reset-race khi reboot mỗi ảnh (image 2+ không boot lại được). */
    if (!boot_pico_once()) {
        xil_printf("PicoRV32 boot FAILED (no STARTED after %d attempts)\r\n",
                   BOOT_ATTEMPTS);
        while (1) {}
    }
    xil_printf("PicoRV32 booted — no-reboot loop ready\r\n");
    ...
    for (int i = 0; i < BENCHMARK_IMAGE_COUNT; i++) {
        /* Stage ảnh + GO + chờ kết quả (no-reboot, FW đã boot sẵn) */
        uint32_t result = run_image(lenet_test_images[i]);
        ...
        if (result == RAAS_MBX_PASS) {
            ...
            /* Ảnh lỗi → tính sai, nhưng FW vẫn trong loop → chạy tiếp ảnh sau */
    ...
    /* 6. Ảnh đã prefill ở DDR — ra lệnh GO + chờ kết quả (FW đã boot sẵn) */
    xil_printf("PicoRV32 booted — triggering inference...\r\n");
    uint32_t result = trigger_and_wait();
